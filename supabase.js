...
const BIN_ID = '6797a5a6acd3cb34a89d3c4c'; // 需要创建
...
class SimpleSupabaseClient {
    ...
    getDefaultPhotos() {
        return [
            { id: '1', title: '城市废墟', category: 'digital', year: 2024, description: '城市中的废墟美学', thumbnail_url: 'https://cdn.hailuoai.com/matrix_agent/20260202/image_tool/output/132600_c469/workspace/desktop_media/6c3e9181-098a-4936-bc1b-969db4e9910e.jpg', image_url: 'https://cdn.hailuoai.com/matrix_agent/20260202/image_tool/output/132600_c469/workspace/desktop_media/6c3e9181-098a-4936-bc1b-969db4e9910e.jpg', is_active: true, sort_order: 1, attributes: { size: '30x40', negative_size: '35mm', other: '' } },
            { id: '2', title: '老街记忆', category: 'film', year: 2023, description: '胶片记录的老街风情', thumbnail_url: 'https://images.unsplash.com/photo-1519111887837-a48ccf9edc00?w=800', image_url: 'https://images.unsplash.com/photo-1519111887837-a48ccf9edc00?w=1600', is_active: true, sort_order: 2, attributes: { size: '40x50', negative_size: '120', other: '限量10张' } },
            { id: '3', title: '静物之美', category: 'wetplate', year: 2024, description: '湿版摄影的独特质感', thumbnail_url: 'https://images.unsplash.com/photo-1494438639946-1ebd1d20bf85?w=800', image_url: 'https://images.unsplash.com/photo-1494438639946-1ebd1d20bf85?w=1600', is_active: true, sort_order: 3, attributes: { size: '20x25', negative_size: '8x10英寸', other: '古典工艺' } },
            { id: '4', title: '自然纹理', category: 'carbon', year: 2023, description: '碳素印相的细腻表现', thumbnail_url: 'https://cdn.hailuoai.com/matrix_agent/20260202/image_tool/output/132438_7a6d/workspace/desktop_media/cc59b835-73cf-40ed-906d-3744f1bb2381.jpg', image_url: 'https://cdn.hailuoai.com/matrix_agent/20260202/image_tool/output/132438_7a6d/workspace/desktop_media/cc59b835-73cf-40ed-906d-3744f1bb2381.jpg', is_active: true, sort_order: 4, attributes: { size: '50x60', negative_size: '4x5英寸', other: '艺术微喷' } },
            { id: '5', title: '植物蓝图', category: 'cyanotype', year: 2024, description: '蓝晒工艺的自然创作', thumbnail_url: 'https://images.unsplash.com/photo-1464822759023-fed622ff2c3b?w=800', image_url: 'https://images.unsplash.com/photo-1464822759023-fed622ff2c3b?w=1600', is_active: true, sort_order: 5, attributes: { size: '30x40', negative_size: '', other: '植物标本' } },
            { id: '6', title: '肖像习作', category: 'vandyke', year: 2023, description: '范戴克棕的人像表现', thumbnail_url: 'https://images.unsplash.com/photo-1531746020798-e6953c6e8e04?w=800', image_url: 'https://images.unsplash.com/photo-1531746020798-e6953c6e8e04?w=1600', is_active: true, sort_order: 6, attributes: { size: '25x30', negative_size: '8x10英寸', other: '手工制作' } },
            { id: '7', title: '夜色都市', category: 'digital', year: 2024, description: '数码摄影的夜景表现', thumbnail_url: 'https://images.unsplash.com/photo-1519501025264-65ba15a82390?w=800', image_url: 'https://images.unsplash.com/photo-1519501025264-65ba15a82390?w=1600', is_active: true, sort_order: 7, attributes: { size: '60x90', negative_size: '全画幅', other: '长曝光' } },
            { id: '8', title: '海浪瞬间', category: 'film', year: 2023, description: '胶片捕捉的海浪瞬间', thumbnail_url: 'https://images.unsplash.com/photo-1505118380757-91f5f5632de0?w=800', image_url: 'https://images.unsplash.com/photo-1505118380757-91f5f5632de0?w=1600', is_active: true, sort_order: 8, attributes: { size: '40x50', negative_size: '120', other: '' } }
        ];
    }
    ...
    async upload(bucket, path, file) {
        ...
        if (file.size > 10 * 1024 * 1024) {
            console.error('[upload] 文件太大，超过 10MB 限制');
        ...
        try {
            ...
            console.error('[upload] 上传过程中发生错误:', error.message);
            console.log('[upload] 将尝试降级方案...');
            console.log('========================================');
            ...
            console.error('💡 请检查：');
            console.error('   1. 存储桶 "photos" 是否已创建？');
            console.error('   2. CORS 配置是否正确？');
            console.error('   3. 存储桶是否为 Public？');
            console.error('========================================');
            
            // 降级为 base64（仅限小文件）
            if (file.size < 500 * 1024) { // 500KB 以下才尝试 base64
                console.log('[upload] 尝试使用 base64 降级方案...');
                return this.uploadAsBase64(path, file);
            } else {
                throw new Error('Supabase Storage 不可用，且文件太大无法使用 base64 存储。请检查 Supabase Storage 配置。');
            }
        }
    }
    
    // 降级方案：将图片存储为 base64（仅用于开发测试）
    async uploadAsBase64(path, file) {
        return new Promise((resolve, reject) => {
            const reader = new FileReader();
            reader.onload = (e) => {
                console.log('[uploadAsBase64] 文件已转换为 base64，长度:', e.target.result.length);
    ...
    async uploadPhoto(file) {
        console.log('[uploadPhoto] 开始上传作品图片...');
        console.log('[uploadPhoto] 文件名:', file.name);
    ...
    async deletePhotoImage(imageUrl) {
        try {
            ...
            if (urlParts.length !== 2) {
                console.error('[deletePhotoImage] 无效的 URL 格式:', imageUrl);
            ...
            if (response.ok) {
                console.log('[deletePhotoImage] 文件已删除:', filename);
                return { error: null };
            } else {
                const errorText = await response.text();
            ...
            console.error('[deletePhotoImage] 删除过程中发生错误:', error);
    ...
    async getAbout() {
        ...
        try {
            ...
            if (localAbout) {
                ...
                console.log('[getAbout] 成功解析关于我数据:', aboutData);
                ...
                console.log('[getAbout] 没有找到数据，返回默认:', defaultAbout);
                return { data: defaultAbout, error: null };
            }
        } catch (error) {
            console.error('[getAbout] 获取关于我数据失败:', error);
    ...
    async saveAbout(data) {
        ...
        console.log('[saveAbout] 收到的原始数据:', JSON.stringify(data, null, 2));
        
        // 检查字段
        console.log('[saveAbout] 所有字段:');
        ...
        try {
            ...
            console.log('[saveAbout] 序列化后的数据总长度:', dataToSave.length, '字符');
            
            console.log('[saveAbout] 尝试保存到 localStorage...');
            localStorage.setItem('about', dataToSave);
            console.log('[saveAbout] ✓ 数据已写入 localStorage');
            
            // 验证保存结果
            const savedData = localStorage.getItem('about');
            console.log('[saveAbout] 从 localStorage 读取验证...');
            
            if (savedData) {
                const parsed = JSON.parse(savedData);
                console.log('[saveAbout] ✓ 验证成功！');
                console.log('[saveAbout] 保存的字段:', Object.keys(parsed));
                console.log('[saveAbout] 小红书二维码:', parsed.xiaohongshu_qrcode ? `有数据 (${parsed.xiaohongshu_qrcode.length} 字符)` : '空');
                console.log('[saveAbout] B站二维码:', parsed.bilibili_qrcode ? `有数据 (${parsed.bilibili_qrcode.length} 字符)` : '空');
                console.log('========================================');
                return { data: data, error: null };
            } else {
                console.error('[saveAbout] ❌ 验证失败，数据未找到');
                console.log('========================================');
                return { data: null, error: { message: '保存验证失败 - 数据未写入 localStorage' } };
            }
        } catch (error) {
    ...
    async getBanners() {
        ...
        try {
            ...
            if (localBanners) {
                ...
                console.log('[getBanners] ✓ 成功加载 Banner 数据，数量:', banners.length);
    ...
    async saveBanners(banners) {
        console.log('========================================');
        console.log('[saveBanners] ★ 开始保存 Banner 数据 ★');
        console.log('[saveBanners] 收到的 Banner 数量:', banners.length);

        try {
            const dataToSave = JSON.stringify(banners);
            console.log('[saveBanners] 序列化后的数据长度:', dataToSave.length, '字符');

            console.log('[saveBanners] 尝试保存到 localStorage...');
            localStorage.setItem('banners', dataToSave);
            console.log('[saveBanners] ✓ Banner 数据已写入 localStorage');
    ...
    async uploadBanner(file) {
        console.log('[uploadBanner] 开始上传 Banner 图片...');
        console.log('[uploadBanner] 文件名:', file.name);
